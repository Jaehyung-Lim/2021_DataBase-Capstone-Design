...
import {selectSql} from "../database/sql";//삽입, 조회에 관련된 쿼리 함수를 불러옵니다.
...
router.post('/', async (req,res)=>{    //값은 req에 저장됩니다.
    ...
    const users=await selectSql.getUsers(); //const라 값을 추가로 할당할 수 없습니다.
//admin은 delete 페이지로, user면 select 페이지로
    ...
    users.map((user) => { //최신 for문 문법
        ...
        if(vars.id===user.Id && vars.password === user.Password){ //필드 만들 때 대문자로 만들어서 이렇게 하는 것
            ...
            else if(vars.id==='nation'){//id가 nation일 때 whoAmI=대통령
                whoAmI = '대통령';
            }
        }
    })
    ...
    else if(checkLogin && whoAmI === '대통령'){
        res.redirect('/deleteNation');
    }
    else{
